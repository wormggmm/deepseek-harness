...
**反向绑定以插件形式随附，而不是作为输入框的一个选项。**`@deepseek-ai/dsh-client-ui-enter-send` 提供服务（Cmd/Ctrl+Enter 提交，其余按键插入换行），并以 `ui-enter-send` 行挂载在随附的 web-app 花名册里，与其余浏览器插件行并列。busy 状态策略原样不动：运行中加速手势仍然插话，空草稿的加速手势仍然把整条队列插进当前轮次，因为插件只决定哪些键能走到提交路径。移除该行（或在 `$DSH_HOME/cordis.patch.yml` 里禁用）即可零改动恢复 Enter 发送。
