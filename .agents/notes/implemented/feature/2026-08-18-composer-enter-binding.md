...
**The inverted binding ships as a plugin, not as a composer option.** A deployment composes its own binding plugin through a profile patch layer — the reference implementation is `@deepseek-ai/dsh-client-ui-enter-send` (Cmd/Ctrl+Enter submits, everything else inserts a line break), carried by the fork that produced this change — and absent any provider the shipped default applies. The busy-state policy is untouched: the accelerated chord still steers while running, and the empty-draft accelerated chord still steers the whole queue, because a binding only decides which keys reach the submission path.
