...
Enter 键本身可以通过可选的 `composerEnterBinding` 服务重新绑定：composer bar 在每次 inject 时用 ctx.get 解析它，并对每个通过 IME 组合与弹层仲裁的 Enter keydown 询问其 `resolve`。没有该服务时使用随附绑定——每个被询问的 Enter 都提交，Cmd/Ctrl 标记加速手势；Shift+Enter 是在任何绑定之前判定的无条件原生换行。[`ui-enter-send`](../ui-enter-send/README.md) 提供反向绑定（Cmd/Ctrl+Enter 提交，普通 Enter 插入换行）；把该插件从 cordis.yml 中摘除即恢复默认。
